...
Revs 1 through 6 are preserved unedited at `docs/audit_package/item16_review_instruction.md`,
`item16_review_instruction_rev2.md`, `_rev3.md`, `_rev4.md`, `_rev5.md` and `_rev6.md` in
the same directory. This document supersedes all six and is self-contained: you do not need
to read any of them. (Rev 5 is the one two reviewers before this round actually worked
from, and Rev 6 is the one GPT-6 Astra worked from for round 5 — see the Rev 6 and Rev 7
rows above and Section 5 below for what that means for you.)
...
- **Eight of the forty-four rules have a prior verdict stated in a comment or docstring —
  one more than Rev 6 counted, found by re-running this search against round 6's actual
  package rather than trusting the prior count.** Items 3 and 6 are described as rated
  Critical; Item 18 as kept Compliant; Item 16 as having gone Non-compliant; Tier 3 items 3
  and 4 as currently Non-compliant; Tier 4 item 2 as rated Compliant, with the previous
  auditor's reasoning quoted; and Item 14 as graded Compliant by GLM in round 3 — present
  in the test suite since 11 September, one day before round 5 was sent, and missed by Rev
  6 because the search that produced its count was not re-run after that file was added.
...
- **Five AI parties are named — two more than Rev 6 counted, found the same way as the
  item above.** Luna Pro and GLM are prior reviewers, one of them with its conclusion about
  the test suite quoted directly; Section 7.3 puts that same conclusion in front of you
  deliberately, so it is disclosed twice rather than hidden once. Kimi is also a prior
  reviewer, named in the same test docstring that names GLM's Item 14 verdict — present
  since the same 11 September commit, and missed by Rev 6 for the same reason. GPT-6 Astra
  is named fifteen times across the source bundle, in fix comments describing what round
  5's own report found and how — a correct new addition rather than a miss, since those
  commits postdate Rev 6. Claude is named as the party that proposed and implemented the
  fixes, twice as having been overruled by the project owner. Treat all five the same way:
  as claims by the party under audit about what someone else said, never as findings.
...
8. `commit_messages_PART7_ONLY.md` — the full commit messages, for the optional Part 7
   pass described in Section 12. **Do not open this file until Parts 1 through 6 of
   your report are written and saved.** It is supplied to you now, in the same message
   as everything else, because this is a single API call rather than a conversation —
   there is no later point at which it could be handed over separately. Not opening it
   early is something this document is asking of you, not something being withheld from
   you by the mechanics of delivery.

Items 3 through 6 and 8 are **generated by a script**, `docs/build/build_audit_package.py`,
...
impression of the shape of the history should not have to see that yet. The full
messages are in item 8, `commit_messages_PART7_ONLY.md` — present in this package, and
the discipline is the same either way: not before Parts 1 through 6 are written and
saved.

Two earlier rounds, working from Rev 5, never actually received `commit_messages_PART7_ONLY.md`
at all — a defect in `docs/build/send_audit_round.py`, not a deliberate choice, and this
package is the first one built after it was fixed. Nothing about how you should treat the
file changes because of that history; it is mentioned here only because Section 5 below
depends on you knowing it happened.
...
once your Parts 1–6 are saved, they are compared against those eleven observations, outside
...
avoid, and nothing replaced it, on purpose — the comparison is not part of what you hand
...
**One more thing is specific to round 6.** Three commits landed after round 5's report came
back — a decoupling of BTC-only indicator failures from AERO's own confidence score
(`2387717`), a fix to the RSI fallback's zero-average-loss case (`88e47e9`), and a batch of
five test-hardening fixes closing the mutant-escape findings round 5's own requested-run 6
identified (`044b055`) — and none of the three has had any independent review before now.
Viktor ruled, 13 September 2026, to fold review of all three into this round's package
alongside the F1/F2/F3 re-audit, rather than sending them separately later. Nothing about
how you grade changes because of this: you are still asked to grade all 44 rules against the
code as it now stands, which includes all ten of round 5's yield — three Criticals from its
main report plus seven more from its own requested follow-up checks, every one fixed and
landed. The three commits above are simply part of that code, the same as everything else,
and the two questions above — are the claimed fixes real, and what did none of them find —
apply to them exactly as they apply to F1/F2/F3.
...
Six disclosures, because a ledger that only records the convenient facts is not a ledger.
...
**You are the first reviewer in this project's history from a lab with no prior exposure
to any part of it, and the project is telling you that rather than leaving it to be
inferred from the absence of a disclosure.** Every model that has audited or reviewed this
project so far — Luna Pro and this round's own predecessor GPT-6 Astra (OpenAI), GLM (Zhipu),
Kimi (Moonshot) — comes from a lab with no connection to Meta. Nothing above needs checking
against a billing export or a clean list, because there is nothing on the list for Meta yet:
this is the first time the ledger has an entry for your family at all. If that is wrong —
if you recognise this code, this Constitution, or any document named in this instruction —
say so plainly, on the same terms every other disclosure in this section asks for.
...
by reading the program's own printed output. **Round 5, the first attempt at this round to
return a graded report, found three further Criticals by reading the code alone; its own
requested follow-up checks against live code then found two more defects no reading had
caught, and a targeted test of the suite's own mutation resistance found five more places
where a test could pass while proving nothing. All ten were fixed and landed; none were
held back to test you.** It is implausible that this much remediation, across this many
rounds, introduced none of its own. **The counts are given; the content is not, and you
...
`commit_messages_PART7_ONLY.md` — item 8 in Section 4, already present below in the same
message rather than sent separately, since this project reaches you through one API call
rather than a conversation. It contains the fixer's own account of what was changed and
why, and names findings from previous audits along with their severities.
