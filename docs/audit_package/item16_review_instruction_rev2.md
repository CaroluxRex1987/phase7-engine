...
Rev 1 is preserved unedited at `docs/audit_package/item16_review_instruction.md`. This
document supersedes it and is self-contained: you do not need to read Rev 1.
...
of Compliant and Non-compliant items, or a list of Critical findings — **stop and say so,
as that refusal did.** You will not be penalised for refusing. You would be doing the job.
...
3. `round2/phase7_engine_source.md` — every module the engine runs, complete, plus the
...
4. `round2/phase7_test_suite.md` — every test module and the runner, complete.
5. `round2/MANIFEST.md` — file counts and a SHA-256 for every file in the two bundles.
6. `round2/version_control_history.md` — the shape of the repository's history.
7. `round2/execution_transcripts.md` — output the engine actually produced when run.
...
`round2/commit_messages_PART7_ONLY.md`, for the optional Part 7 pass described in
...
You are **Qwen3.8-Max**. This project keeps a written ledger of which model families have
...
Three disclosures, because a ledger that only records the convenient facts is not a ledger.
...
`round2/execution_transcripts.md` contains output the engine actually produced, captured
...
`round2/commit_messages_PART7_ONLY.md`. It contains the fixer's own account of what was
