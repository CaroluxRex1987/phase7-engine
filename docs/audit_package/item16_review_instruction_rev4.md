...
observations. Those were verified and fixed; Section 4a explains that decision and Section
13 gives you the material.
...
**If you are the same model as the attempt that returned the eleven observations, say so in
your report.** Its eleven
observations are among the material you will be given in Section 13, and a reader should be
able to see that the comparison there may be a model against its own earlier reasoning
rather than against an independent one.
...
Parts 7 and 8 come afterwards, from material you may not open until Parts 1–6 are saved.
Sections 12 and 13 describe them. **Parts 1–6 are the report.** If you stop there, the
round has succeeded; Parts 7 and 8 are worth having and are not what the audit is for.
...
content is not, and you should not ask for it before Parts 7 and 8.**
...
## 13. Part 8 — the eleven prior observations, and what you are being asked about them

There is a second withheld file, `prior_observations_PART8_ONLY.md`. It holds the raw
reasoning of an earlier attempt at this round — the one that was given a superseded
revision of this document and no Constitution, refused to grade the rules, and ran Section
7's checks anyway. Its eleven observations are in it verbatim, unedited, including the
parts the project judged wrong. The check numbers it cites are the same ones this document
uses: Section 7 was byte-identical between the revision it received and Rev 3. Only 7.1 and
7.6 have changed since, and both gained material rather than losing any — so every check it
ran is a check you were also asked to run.

**The same rule as Part 7 applies, and for the same reason: do not open it until Parts 1–6
are written and saved.** It is a second reader's view of code you are grading, and it would
replace your view with theirs.

Afterwards, produce **Part 8 — Prior-observation reconciliation**, four short lists:

1. Observations you also found, and where you reported them.
2. Observations you did not find. **For each, say why your method did not reach it** — the
   file was not in a place you looked, the reasoning needed execution, you saw the code and
   judged it acceptable, or you have no explanation. "No explanation" is a legitimate and
   useful answer; a reconstructed one is not.
3. Observations you believe are **wrong**, with your reasoning. They were accepted and
   fixed, and that does not make them correct. Disagreeing here is a finding.
4. Things you found that it did not.

**What this is for, stated plainly so you can judge whether to participate.** The project
uses the comparison to calibrate how much weight a single audit of this codebase deserves.
It is a measurement of the review process, not a mark against you: list 2 being long is
informative and is not a failure, and a Part 8 that quietly stretches to make list 1 longer
destroys the only thing the exercise produces.

Two things that would corrupt this if you did them, so do not:

- **Do not revise Parts 1–6 after reading this file.** If it changes your mind about a
  verdict, say so in Part 8 and leave the verdict standing. A report edited to agree with
  material it was supposed to be independent of is worth nothing, and the edit would be
  invisible.
- **Do not treat the eleven as a scoring key.** They came from a run that graded no rule,
  against a version of the code that no longer exists. They are one reader's observations,
  and you are the one with the standard in front of you.
