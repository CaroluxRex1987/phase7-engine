...
Revs 1 through 5 are preserved unedited at `docs/audit_package/item16_review_instruction.md`,
`item16_review_instruction_rev2.md`, `_rev3.md`, `_rev4.md` and `_rev5.md` in the same
directory. This document supersedes all five and is self-contained: you do not need to
read any of them. (Rev 5 is the one two other reviewers actually worked from — see the
Rev 6 row above and Section 5 below for what that means for you.)
...
Five disclosures, because a ledger that only records the convenient facts is not a ledger.
