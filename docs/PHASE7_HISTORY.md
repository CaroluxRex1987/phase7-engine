...
## 26 September 2026 (eighth session) — PHASE7_NEXT.md as it stood at `75682ee`

*Moved here verbatim on 26 September 2026, when PHASE7_NEXT.md was rewritten by the
eighth session's first commit — the once-per-session rewrite. The eighth session is a new
chat on the same day as the seventh. Every line below the rule is the file's content at
`75682ee`, unchanged, including its own wording: "this session" is the seventh,
26 September session, and "the commit that writes this line" names `75682ee`. Its
headings are demoted one level (`#` → `##`, `##` → `###`) so they nest under this entry;
no other character is changed.*
...
## 26 September 2026 (ninth session) — the repository moved to `E:\phase7_engine`

*A new dated entry. Every earlier record that names `D:\phase7_engine` or
`D:\Phase_7_Engine_Random_Files` was true when written and is not edited (DECISIONS,
"Ruling, 20 September 2026 — dated records are not edited to follow a move").*

**What Viktor decided.** Before phase 3 of the roadmap started, Viktor had made two new
drives on his machine, `Phase_7_Engine (E:)` (9.76 GB) and `Phase_7_Engine_Random_Misc
(G:)` (4.88 GB), and asked for the repository to move to E: and the files kept outside
it to G:. Asked three questions, he chose Claude's recommended option on each: keep the
folder names (`E:\phase7_engine`, `G:\Phase_7_Engine_Random_Files`), so a path changes
only by its drive letter; leave `D:\USB Backup\Phase7_Engine documents` where it is —
it is a backup made before the reinstall, and it holds the plaintext API key file; and,
once the copies were verified, rename the originals rather than delete them, so anything
still pointing at the old path fails instead of working on a stale copy. Deleting the
renamed originals is his, later.

**How.** Copy, verify, then rename; nothing was moved in one step. Viktor ran
`robocopy <source> <destination> /E /COPY:DAT /DCOPY:DAT /R:1 /W:1 /NFL /NDL` for each
folder. The repository: 83 directories, 666 files, 23.69 MB copied, 0 failed, 0 mismatch.
The Random Files: 7 directories, 25 files, 663.2 KB, 0 failed (predicted: 25 files, 7
directories, 0 failed).

**What was checked.**

- On Windows, by Viktor, in `E:\phase7_engine` (his paste): `git status --short` printed
  nothing; `git fsck --full --no-dangling` printed progress lines only (2071 objects, no
  error); `git log -1 --format=%H` printed `eba6a2a0d38deccdc7b3079b5bfa6de58946286c`;
  `python -m pytest -q` printed 597 passed. The copies' timestamps differ from the
  index's, so that `git status` re-read every tracked file: it checks their content, not
  only their presence.
- By Claude, over the device bridge, the files git does not check: the 91 ignored data
  files in the repository — `logs/` (the decision log, the engine log, the state file,
  the chart and 28 raw-candle archives) and `docs/audit_package/round2` to `round6`
  beyond each tracked MANIFEST (including the round-2 Kimi and round-3 GLM responses) —
  and all 25 Random Files were staged from both drives and compared by SHA-256:
  identical, 91 of 91 and 25 of 25. Negative control: one byte appended to a copy of one
  file changed its hash. `__pycache__` and `.pytest_cache` were not compared; they are
  rebuilt. Claude listed every directory of the repository on E: except `.git` and the
  caches, and found no other file that git ignores; the push of `eba6a2a` had found no
  untracked file (section 1 "none").
- Then Viktor renamed the originals: `D:\phase7_engine_MOVED_TO_E` and
  `D:\Phase_7_Engine_Random_Files_MOVED_TO_G` (Claude's listing of D: afterwards). The
  first `ren` failed with "being used by another process": his cmd window was inside the
  folder. It succeeded after `cd /d E:\phase7_engine`.

**What the move does not change** — read from the code, 26 September:

- `code_hash`: the walk records each file's path relative to the repository root
  (`core/code_fingerprint.py`, `iter_source_files` and `code_fingerprint`), so the drive
  does not enter it. Computed on a clone of `eba6a2a` under Python 3.12.3:
  `b3c2308f…`, unmoved.
- The pre-push hook: `core.hooksPath = githooks` is a relative path in `.git/config`,
  which was copied with the repository.
- The engine's output: `LOG_DIR = "logs/"` and `CHART_DIR = "logs/charts/"` are relative
  (`core/config.py:65–66`), so the engine must be run from `E:\phase7_engine`. No
  virtualenv lives inside the repository.
- No tracked code or test depends on the drive. Outside dated records, two tracked files
  name `D:\phase7_engine`: a usage example in `docs/build/session_handover_check.py`'s
  docstring, and a sentence in `tests/test_clean_checkout.py`'s docstring describing
  Viktor's disk when that test was written. Neither was changed; the first is an example
  of an argument, the second is a statement about its own date.

**Not checked:** whether E: and G: are partitions of the same physical disk as D: — if
they are, they add no protection against a disk failure, and GitHub and the USB backup
remain the backups; and any shortcut, editor workspace or scheduled task on Viktor's
machine that names the old path.

**Outside the repository:** the patch-delivery skill named `D:\phase7_engine` in its
description and its delivery step; Claude proposed the update to Viktor the same day.
Documents in the Random Files that name the old paths are dated records and are not
edited.


## 26 September 2026 (ninth session) — PHASE7_NEXT.md as it stood at `eba6a2a`

*Moved here verbatim on 26 September 2026, when PHASE7_NEXT.md was rewritten by the
ninth session's first commit — the once-per-session rewrite. The ninth session is a new
chat on the same day as the eighth. Every line below the rule is the file's content at
`eba6a2a`, unchanged, including its own wording: "this session" is the eighth,
26 September session, and "the commit that writes this line" names `eba6a2a`. Its
headings are demoted one level (`#` → `##`, `##` → `###`) so they nest under this entry;
no other character is changed.*
...
## 26 September 2026 (tenth session) — phase 3 started: the order, finding 16 ruled, and a Windows test-run episode

*A new dated entry, filed with the tenth session's first commit.*

**The phase-3 order.** Phase 3 of the roadmap to the audit ("Phase 7 roadmap to the
independent audit", a Claude Docs document outside the repository) is Viktor's findings
4, 5, 6, 7, 16 and 18 and Claude's work order G. Claude proposed the order 16, 7, 4, 6,
5, 18, then G last, under the 21 September delegation of the order of work; Viktor
agreed it on 26 September. One finding at a time, at his pace: he writes his position
first, Claude critiques it, and Claude drafts the DECISIONS wording once he has ruled.

**Finding 16.** Claude re-read `data/data_fetcher.py` off Viktor's disk before asking
for his position and put four questions to him. Viktor's position: the last closed
candle (1), and yes to recording the candle in the log (4); he asked for Claude's
suggestion on the panel (2) and on staleness (3), and adopted both. He approved the
DECISIONS wording as drafted. The ruling is in DECISIONS, "Ruling, 26 September 2026 —
decisions are made on closed candles (finding 16)". Its code has not landed.

**A Windows test-run episode, before `7d103d7` was committed.** Reported by Viktor at
the start of this session. In a new, non-administrator cmd window (a new taskbar
shortcut starting in `E:\phase7_engine`), `python -m pytest -q` gave 590 passed,
7 errors. All seven were `PermissionError [WinError 5]` on pytest's temp folder
`C:\Users\MSI-PC\AppData\Local\Temp\pytest-of-MSI-PC`, in
`test_fetch_and_import_hygiene.py` (2), `test_risk_verdict_is_read_not_assumed.py` (2)
and `test_router_btc_seam.py` (3) — errors of the environment, not failures of the
engine. Viktor removed the folder from an administrator prompt; the rerun gave 597
passed, and `run_tests.py` and the push were then as predicted. Claude's inference, not
confirmed: the folder had been created by Viktor's old, elevated cmd window, so a
non-elevated process could not write inside it.

**Outside the repository.** The patch-delivery skill now names `E:\phase7_engine`;
Viktor updated and saved it on 26 September. The renamed originals on D:
(`phase7_engine_MOVED_TO_E`, `Phase_7_Engine_Random_Files_MOVED_TO_G`) are still his to
delete.


## 26 September 2026 (tenth session) — PHASE7_NEXT.md as it stood at `7d103d7`

*Moved here verbatim on 26 September 2026, when PHASE7_NEXT.md was rewritten by the
tenth session's first commit — the once-per-session rewrite. The tenth session is a new
chat on the same day as the eighth and ninth. Every line below the rule is the file's
content at `7d103d7`, unchanged, including its own wording: "this session" is the ninth,
26 September session, and "the commit that writes this line" names `7d103d7`. Its
headings are demoted one level (`#` → `##`, `##` → `###`) so they nest under this entry;
no other character is changed.*
...
## 26 September 2026 (eleventh session) — PHASE7_NEXT.md as it stood at `bf2e802`

*Moved here verbatim on 26 September 2026, when PHASE7_NEXT.md was rewritten by the
eleventh session's first commit — the once-per-session rewrite; that commit is finding
16's code. The eleventh session is a new chat on the same day as the eighth, ninth and
tenth. Every line below the rule is the file's content at `bf2e802`, unchanged,
including its own wording: "this session" is the tenth, 26 September session, and "the
commit that writes this line" names `bf2e802`. Its headings are demoted one level (`#` →
`##`, `##` → `###`) so they nest under this entry; no other character is changed.*
