...
| after the direction-box tests | 21 Sep | `core/decision_log.py`: non-finite floats written as `null` with `allow_nan=False` (19); `read_with_report()`, and `read()` logs what it skips (20); the module docstring names the recorded source, `"pinned"` (21) | deferred-read findings 19–21 | `test_decision_log_record_format.py` | none on the decision; the log's spelling of "no value" is `null` from this commit on |
| after the decision-log commit | 21 Sep | `core/decision_log.py`: `DecisionModel.BTC_STRESS_PENALTY`, `AVG_REWARD_R`, `EV_BREAKEVEN_BAND_R` added to `FINGERPRINTED_MODULES` | deferred-read finding 22 | `test_fingerprint_names_every_constant.py` (scans every fingerprinted module, so a future omission fails too) | **golden re-baselined**: `run_hash` (2 places) and the archive name (2 places) move; 3 leaves added under `module_constants`; no decision field |
