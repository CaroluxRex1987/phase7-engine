...
| Tests | 522 pass with `pandas_ta` installed (390 pass, 121 skip without it). The fixture-free runner (`run_tests.py`) reports 451 passed, 0 failed, 32 errors — all from tests written with pytest fixtures that runner deliberately doesn't support, not defects; see its own docstring. |
...
**All tests currently pass.** 522 with `pandas_ta` installed, 390 (plus 121 skipped)
