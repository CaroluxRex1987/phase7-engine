...
| Tests | 589 pass with `pandas_ta` installed (452 pass, 126 skip without it). The fixture-free runner (`run_tests.py`) reports 518 passed, 0 failed, 32 errors — all from tests written with pytest fixtures that runner deliberately doesn't support, not defects; see its own docstring. |
...
**All tests currently pass.** 589 with `pandas_ta` installed, 452 (plus 126 skipped)
