...
| Tests | 515 pass with `pandas_ta` installed (383 pass, 121 skip without it). The fixture-free runner (`run_tests.py`) reports 444 passed, 0 failed, 32 errors — all from tests written with pytest fixtures that runner deliberately doesn't support, not defects; see its own docstring. |
...
**All tests currently pass.** 515 with `pandas_ta` installed, 383 (plus 121 skipped)
