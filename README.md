...
| Tests | 549 pass with `pandas_ta` installed (414 pass, 124 skip without it). The fixture-free runner (`run_tests.py`) reports 478 passed, 0 failed, 32 errors — all from tests written with pytest fixtures that runner deliberately doesn't support, not defects; see its own docstring. |
...
**All tests currently pass.** 549 with `pandas_ta` installed, 414 (plus 124 skipped)
